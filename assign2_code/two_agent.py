...
class Discretizer(gym.ActionWrapper):
    ...
    def __init__(self, env, combos):
        ...
        buttons.append(['UP', 'DOWN',  'SELECT', 'RESET', 'NONSENSE',
                       None, 'LEFT', 'RIGHT', 'NONSENSE', 'BUTTON'])
...
class retroActionWrapper(gym.ActionWrapper):
    ...
    def action(self, action):
        ...
        if self.agent_num == 1:
            ...
            combined_act = ACTION_MEANINGS[action[0]
                ] + 18 * ACTION_MEANINGS[action[1]]
            print(combined_act)
...
class HumanAgent(object):
    ...
    def main(self):
        ...
        while True:
            #a1 = self.wt.act(ob)
            if i % skip_frames == 0:
                a1 = self.ts.action_ts(wrapped_ob)
            if self.rollout == 0:
            ...
            if i % skip_frames == 1:
                ...
                self._frame_buffer.append(max_frame)
            wrapped_ob = np.concatenate(self._frame_buffer, 2)  
...
if __name__ == "__main__":
    ...
    # eps = list(np.linspace(0.9, 0.0, num=10))
    # print(eps)
    # for ep in eps:
    #     for i in range(5):
    #         ob = test_env.reset()
